export class Canvas {
    ...
    drawRoundRect(position, width, height, radius, color, strokeColor, strokeWidth = 2) {
        const ctx = this.ctx;
    ...
    drawCircleStroke(position, radius, color, lineWidth = 2) {
    ...
    drawText(text, position, color, font = "20px monospace", textAlign = "left") {
        const ctx = this.ctx;
        ctx.fillStyle = color;
        ctx.font = font;
    ...
    drawTextShadow(text, position, color, shadowColor, font = "20px monospace", textAlign = "left") {
        const ctx = this.ctx;
        ctx.save();
        ctx.font = font;
        ctx.textAlign = textAlign;
    ...
    drawTank(cx, cy, aimAngle, isMe, color) {
        const ctx = this.ctx;
        ctx.save();
        ctx.translate(cx, cy);
    ...
    drawBullet(cx, cy, radius, activated) {
        const ctx = this.ctx;
        ctx.save();
        if (activated) {
