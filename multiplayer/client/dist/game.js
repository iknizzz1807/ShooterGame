...
export class Game {
    ...
    positionSearchInput(canvasX, canvasY, canvasW, canvasH) {
        if (!this.searchInput)
            return;
    ...
    handleServerMessage(msg) {
        ...
        switch (msg.type) {
            ...
            case "room_list":
                ...
                break;
            case "error":
                console.error("Server error:", (_a = msg.payload) === null || _a === void 0 ? void 0 : _a.message);
                break;
            case "gameState":
                ...
                this.timeRemaining = (_b = sp.timeRemaining) !== null && _b !== void 0 ? _b : 0;
                this.shootCooldownMax = (_c = sp.shootCooldownMax) !== null && _c !== void 0 ? _c : 2;
                this.amIReady = this.myPlayerId
                    ? sp.readyPlayers[this.myPlayerId] || false
                    : false;
                const pIds = new Set(Object.keys(sp.players || {}));
                this.players.forEach((_, id) => { if (!pIds.has(id))
                    this.players.delete(id); });
                pIds.forEach((id) => {
                    const s = sp.players[id];
                    if (this.players.has(id))
                        this.players.get(id).updateState(s);
                    else
                        this.players.set(id, new Player(s));
                });
                const bIds = new Set(Object.keys(sp.bullets || {}));
                this.bullets.forEach((_, id) => { if (!bIds.has(id))
                    this.bullets.delete(id); });
                bIds.forEach((id) => {
                    const s = sp.bullets[id];
                    if (this.bullets.has(id))
                        this.bullets.get(id).updateState(s);
                    else
                        this.bullets.set(id, new Bullet(s));
                });
                break;
        }
    }
    ...
    setupInputHandlers() {
        ...
        (_a = this.canvas.getCanvas()) === null || _a === void 0 ? void 0 : _a.addEventListener("mousemove", (e) => {
            ...
            this.mousePosition = new Vector2D((e.clientX - r.left) * scaleX, (e.clientY - r.top) * scaleY);
        });
        (_b = this.canvas.getCanvas()) === null || _b === void 0 ? void 0 : _b.addEventListener("wheel", (e) => {
            if (this.clientState === "lobby") {
                this.lobbyScrollOffset = Math.max(0, this.lobbyScrollOffset + e.deltaY * 0.5);
                e.preventDefault();
            }
        }, { passive: false });
        (_c = this.canvas.getCanvas()) === null || _c === void 0 ? void 0 : _c.addEventListener("click", (e) => {
            const r = this.canvas.getCanvas().getBoundingClientRect();
            const scaleX = this.canvas.getWidth() / r.width;
            const scaleY = this.canvas.getHeight() / r.height;
            const pos = new Vector2D((e.clientX - r.left) * scaleX, (e.clientY - r.top) * scaleY);
            if (this.clientState === "lobby")
                this.handleLobbyClick(pos);
            else if (this.clientState === "in_game")
                this.handleInGameClick(pos);
        });
    }
    handleLobbyClick(pos) {
    ...
    drawMilButton(cx, cy, w, h, label, variant = "green", pulse = false) {
    ...
    drawLobby() {
        ...
        ctx.setLineDash([6, 4]);
        ctx.beginPath();
        ctx.moveTo(cx - 340, 105);
        ctx.lineTo(cx + 340, 105);
        ctx.stroke();
        ctx.setLineDash([]);
        ...
        // Section label
        const filteredRooms = this.rooms.filter(r => !this.searchQuery ||
        ...
        // ── Footer ───────────────────────────────────────────────
        this.canvas.drawText("WASD: Move  |  Mouse: Aim  |  Click: Shoot  |  Scroll: Browse rooms", new Vector2D(cx, H - 28), "#3a4a2a", "12px monospace", "center");
    }
    // --- IN-GAME DRAWING ---
    drawPlayers() {
        this.players.forEach((player) => {
            ...
            if (isMe && this.mousePosition) {
                aimAngle = Math.atan2(this.mousePosition.y - cy, this.mousePosition.x - cx);
            }
            ...
            this.canvas.drawTextShadow(label, new Vector2D(cx, by - 4), labelColor, isMe ? "#406020" : "#601010", "bold 12px monospace", "center");
            // Aiming laser — only for me
            if (isMe && this.mousePosition && this.roomState === "in_progress") {
                const dir = new Vector2D(this.mousePosition.x - cx, this.mousePosition.y - cy).normalize();
    ...
    drawInGameUI() {
        ...
        this.canvas.drawText(cdReady ? "READY" : `${cd.toFixed(1)}s`, new Vector2D(107, 55), cdReady ? "#80ee80" : "#ccaa44", "11px monospace", "center");
    }
    drawTimer() {
        ...
        this.canvas.drawText(`${String(Math.floor(secs / 60)).padStart(2, "0")}:${String(secs % 60).padStart(2, "0")}`, new Vector2D(cx, 33), color, `bold 20px monospace`, "center");
    }
    drawWaitingScreen() {
        const ctx = this.canvas.getCtx();
        const cx = this.canvas.getWidth() / 2;
        ...
        if (this.players.size < 2) {
            ...
            this.canvas.drawTextShadow("WAITING FOR OPPONENT" + ".".repeat(dots), new Vector2D(cx, cy - 10), "#c8d860", "#406020", "bold 22px monospace", "center");
            ...
            if (this.currentRoomId) {
                ...
                ctx.lineWidth = 1;
                ctx.strokeRect(chipX, chipY, chipW, chipH);
                this.canvas.drawText(`📋 Room ID: ${shortId}  (click to copy)`, new Vector2D(cx, chipY + 17), "#7aaa50", "12px monospace", "center");
                // Register click area
                this._waitingCopyArea = { x: chipX, y: chipY, width: chipW, height: chipH };
            }
        }
        else if (this.amIReady) {
            this.canvas.drawTextShadow("READY — WAITING FOR ENEMY", new Vector2D(cx, cy - 10), "#c8d860", "#406020", "bold 22px monospace", "center");
            this.canvas.drawText("Enemy is preparing...", new Vector2D(cx, cy + 30), "#557744", "15px monospace", "center");
        }
        else {
            this.canvas.drawTextShadow("CLICK TO READY UP", new Vector2D(cx, cy - 10), "#80ee80", "#206010", `bold ${22 + 2 * Math.sin(this.animTime * 3)}px monospace`, "center");
    ...
    drawGameOver() {
        ...
        this.canvas.drawTextShadow(headline, new Vector2D(cx, cy - 28), headlineColor, glowColor, "bold 52px monospace", "center");
        this.canvas.drawText(isDraw ? "No winner this round" : isWin ? "Enemy tank destroyed!" : "Your tank was destroyed", new Vector2D(cx, cy + 18), "#aaa", "16px monospace", "center");
        // Pulsing restart prompt
        const alpha = 0.6 + 0.4 * Math.sin(this.animTime * 3);
        this.canvas.drawText("[ CLICK TO REMATCH ]", new Vector2D(cx, cy + 60), `rgba(180,220,120,${alpha})`, "bold 18px monospace", "center");
    }
    drawInGameContent() {
        switch (this.roomState) {
            case "waiting":
    ...
    drawConnecting(cx, cy) {
        const dots = ".".repeat(Math.floor(this.animTime * 2) % 4);
        this.canvas.drawTextShadow("CONNECTING" + dots, new Vector2D(cx, cy), "#c8d860", "#406020", "bold 28px monospace", "center");
    }
    drawDisconnected(cx, cy) {
        this.canvas.drawTextShadow("CONNECTION LOST", new Vector2D(cx, cy - 20), "#ee4444", "#601010", "bold 32px monospace", "center");
        this.canvas.drawText("Refresh the page to reconnect", new Vector2D(cx, cy + 20), "#886666", "18px monospace", "center");
    }
    startGame() {
        requestAnimationFrame(() => this.gameLoop());
    }
